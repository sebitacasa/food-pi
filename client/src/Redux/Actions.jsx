...
export function getAllRecipes(){
    return async (dispatch) => {
        let json = await axios.get(`http://localhost:3001/recipes/getAll`)
...
export function getByName(title){
    return async (dispatch)=>{
        let jsonName = await axios.get(`http://localhost:3001/recipes?title=${title}`)
        return dispatch({
            type: GET_BY_NAME,
...
export function getById(id){
    return async(dispatch)=>{
        let jsonId = await axios.get(`http://localhost:3001/recipes/${id}`) 
        return dispatch({
            type: GET_BY_ID,
            payload: jsonId.data
        })
...
export function getTypeDiet(){
    return async(dispatch)=>{
        let jsonTypeDiet = await axios.get("http://localhost:3001/diets/")
        return dispatch({
            type: GET_TYPE_DIET,
...
export function postRecipe (payload){
    return async () => {
        const recip = await axios.post('http://localhost:3001/recipes/post', payload) 
        return {
            type: CREATE_RECIPE,
            payload: recip.data
        }
